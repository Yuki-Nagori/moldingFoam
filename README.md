...
**v1.30**（任务 063：闭合、耗散与重启状态一致性；完整契约回归待 CI）：

- 修正纤维 `closure hybrid` 的线性/二次闭合权重；
  `conductivityAnisotropy` 必须在 **(-1.5, 3)** 内，越界在初始化时报错。
- Cross-WLF 的结晶/取向黏度增强统一用于内部场、边界场及黏性生热。
  启用这些增强的历史温升/壁面力结果需要重新验证。
- 排气口按 **patch 名分别密封**；某一出口到达熔体不再关闭其他出口。
  同一个 patch 内仍整片关闭，需要独立控制的出口应使用不同 patch。
- 时间目录新增 `moldingCycleState`（逐出口密封、周期编号/起点、初始时间步）、
  启用预算/修正时的 `moldingMassBudget`，以及启用汽蚀闭锁时的
  `moldingVoidRhoRef`。重启必须保留完整时间目录。
- `nCycles > 1` 首次运行保存 `constant/moldingInitial/`；重启必须保留该
  目录及原分解/网格。缺失快照会明确报错，避免把已填充的中途解当作新周期
  初态。单周期不分配这些快照。周期重置同步重置预算、应力和空洞参考状态。
- 入口边界写出流道、浇口、总流量与 Function1 流量曲线；压力边界写出
  runner、relaxation 与升压起点。旧单周期状态仍可读取，但旧文件未保存的
  历史不能补算；依赖这些历史的续跑应从修复后的完整检查点开始。
- 并行 alpha 通量诊断去掉重复归约；修正的是诊断打印，不是旧契约质量
  误差的计算。新增 `xmake run reviewRegression` 有界回归和
  `viscoelasticFeedback` 单步应力反馈用例，均纳入 nightly。
- 工艺曲线/阀时序仍使用原有绝对时间语义；本次不改变其解释，也未实现
  任务 062 的逐浇口指定流量。下述 v1.29 数字保留为历史基线。
