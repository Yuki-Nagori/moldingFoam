...
**为什么当时不加"逐浇口 Function1"**（**2026-09-15 补记：这条理由有一半是错的**）：
当时的论据是"逐浇口目标在边界层已可表达"，对**压力**成立（每浇口 patch 的
`0/p_rgh` 保压目标 ✓），对**流量不成立**——有 `runner` 时 patch 上的
`volumetricFlowRate(Profile)` 被当作**网络总流量**，不是该浇口自己的流量。
因此"流量受控浇口"确实需要网络内支持；已立为 **062**（含语义、两个求解器
的改法、阀门与给定流量的优先级、无自由浇口时的退化定义、测试与验收）。
要改先后顺序仍用阀时序、改分配仍调直径/长度 ✓ 这两条不变。
