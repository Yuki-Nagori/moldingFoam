...
#******************************************************************************
#******************************************************************************
# Acceptance verification for the anisotropic free-shrinkage quarter
# bar (task 034 tensor-eigenstrain follow-up): a 30 x 1 bar whose x = 0
# and y = 0 planes are rollers (slip) and whose other two faces are
# traction-free, carrying a uniform free eigenstrain
# diag(eps_x, eps_y, 0) = diag(0.02, 0.005, 0) imposed through the
# moldingTractionDisplacement boundary condition (the solid thermal path
# is disabled by T = 0, so only the tensor eigenstrain acts).
#
...
#
# Usage: verify-aniso-shrink-bar.py <caseDir>   (exits non-zero on failure)
#******************************************************************************
...
def main():
    ...
    for j in range(NY):
        for i in range(NX):
            ...
            x = (i + 0.5)*L/NX
            y = (j + 0.5)*H/NY
            maxErrX = max(maxErrX, abs(ux - EPSX*x))
            maxErrY = max(maxErrY, abs(uy - EPSY*y))

    relX = maxErrX/(EPSX*L)
    relY = maxErrY/(EPSY*H)
    print("  linearity: max|u_x - eps_x x| = {:.3e} m ({:.2e} of eps_x L)".format(
        maxErrX, relX))
    print("  linearity: max|u_y - eps_y y| = {:.3e} m ({:.2e} of eps_y h)".format(
        maxErrY, relY))
    ...
    if te is None:
        ...
        print("  free end u_x = {:.6e} m (expected {:.6e}, spread {:.2e})"
              .format(mean, EPSX*L, spread))
        if abs(mean - EPSX*L)/(EPSX*L) > TOL or spread/(EPSX*L) > TOL:
    ...
    if ts is None:
        ...
        print("  free surface u_y = {:.6e} m (expected {:.6e}, spread {:.2e})"
              .format(mean, EPSY*H, spread))
        if abs(mean - EPSY*H)/(EPSY*H) > TOL or spread/(EPSY*H) > TOL:
    ...
    print("PASS: the uniform anisotropic eigenstrain is reproduced exactly "
          "about the rollers (stress-free)")
