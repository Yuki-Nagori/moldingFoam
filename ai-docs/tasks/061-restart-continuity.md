...
## 补记：2026-09-15 覆盖边界复核

原 done 仅代表上述载体的验收，不代表所有可选模型/周期组合可无损恢复。
063 新增逐出口、周期快照/预算与汽蚀参考密度持久化，并修复入口流道/曲线
和压力升压历史漏写；见 [063](063-review-closure-state-consistency.md)。
非零首次启动、完整升压中段、非零 tau、多区域/分解与缺失检查点语义见
[064](064-checkpoint-cycle-contract.md)。保留原验收数字，不将小载体结果
扩展为这些路径已验证的结论。
