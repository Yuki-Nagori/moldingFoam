...
- **领先假设**：压力方程的质量通量用**步初密度**
  （`alphaRhoPhi1 = interpolate(rho1_old)·alphaPhi1`，在 prePredictor
  构造），而域质量用步末密度（`correctRho` 后）——一步滞后使每步误差
  ∼(Δρ·flux)，N 步累积与 dt 无关（N∼1/dt）→ 与观测一致；静态网格下
  模块的 `ddtCorr`（`Uf` 为空）不激活，缺少密度时变通量修正；
- **候选修法**：压力方程的面密度用 CN/隐式（`rho_mid/new`）或加
  密度时变通量修正（ddtCorr 风格）；实现需覆写 `pressureCorrector`
  （上游 ~210 行；实验可复制修改，收敛后再评估上游化/重构）。
