...
- **全套回归（2026-09-10，近期求解器改动后）**：模型测试 ✓；
  18 求解器用例 ✓；couette 4.14e-4 / couetteSlip 3.3e-9 / stefan
  （前沿 <1%、profile 0.34 K）✓；moldCHT 5 案例（界面 0.19%、
  腔体 1.33%、充填能量/体积、周期、002 极限 1.05e-4）✓；
  `validation/thermoelastic` 5.46% ✓；**并行契约 4 进程
  9.383e-04**（与基线一致）✓。
