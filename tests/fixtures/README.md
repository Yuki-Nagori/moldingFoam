...
`dual-domain-v1.sample.json` 是公开 CI 使用的合成薄壁中面（10 mm × 10 mm、
4 节点、2 三角形、逐面厚度 1 mm）。真实 Mug STL 导出在本地验证为
...
两面整体同时翻转不能靠相邻绕序检查辨别。
该 fixture 可用于平面薄壁输入/解析解起步，但仍没有双面匹配、内外法向、边界和积分规则，
不能替代 Mug 完整精度验收。真实 Mug 导出同样必须先补齐这些语义；
参考规模 33,418/66,830 也未达到。
...
若要测试本地真实导出，直接把路径传给读取器，不覆盖仓库 fixture：

```bash
python3 scripts/dual_domain_input.py /private/tmp/mug-dual-domain-v1.json
```
