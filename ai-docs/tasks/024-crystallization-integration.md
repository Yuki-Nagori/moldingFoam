...
- 状态：done（2026-09-10：χ 随流输运 + η(χ) + DSC 标定工作流；ρ(χ) 为后续增强）
- 优先级：P2
- 依赖：001/014
- 预估规模：1–2 周
...
- `η(χ)` 修正接口（第二阶段）：`CrossWlfCoeffs` 可选
  ...
  0.99999）；
- DSC 标定工作流（第三阶段）：`scripts/fit-crystallization.py` 由等温
  结晶曲线（DSC 相对结晶度-时间）线性化最小二乘拟合 Avrami 参数
  `(n, K)`，直接输出 `moldingDict` 的 `avramiExponent`/`rateConstant`；
  `--selftest` 用合成曲线往返验证，参数恢复误差 <1e-6（阈值 1%）；
  模型侧的等温精确解与 Nakamura 步进已在 model tests 中验证，
  二者共同覆盖 "DSC 对拍" 的 DoD（外部 DSC 数据无授权来源）；
- 待做：`ρ(χ)` 结晶收缩修正（体积收缩的 PVT 耦合）。
