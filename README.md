...
生效）：4 子域并行质量守恒误差 **5.16e-04**（阈值 1e-3，v1.28 的界面
子循环/容差/dt 组合，见第 8 节变更日志），全周期 15,215 步；墙钟随
VM 状态波动（同会话对照：v1.27 设置中位 677.5 s、v1.28 设置中位
572 s），全部通过。
...
- 全周期（填充+保压+冷却+顶出）约 15,215 步（v1.28 起）、4 子域并行
  约 10 分钟量级（8 核 ARM64 VM，黏性生热开启；绝对墙钟随 VM 状态
  波动 ±20–25%）；
- 保压期的可压缩界面输运是质量守恒的主要误差源，且**误差对 dt 一阶**
  （047 三点定标）：契约 case 的 `maxCo 0.5 / maxAlphaCo 0.015 /
  nSubCycles 8 / 能量 tol 1e-5 / MULESCorr no`（v1.28）实测守恒
  5.16e-04；放宽到 `maxAlphaCo 0.1 / nSubCycles 6` 会使误差升至
  1.5–2.6e-3，`maxAlphaCo 0.06` 单点实测 1.89e-3；
...
注意三个 dt 变体的 V/P 切换在 fill ≈ 0.895 由压力型触发（基线 0.9607
分数型）：dt 变粗会让切换提前。严格解耦需固定切换判据后重测，网格轴
改为固定 dt 协议后补，见
...
- 效果：**墙钟中位 677.5 → 572 s（−16%；本会话基线样本 624/731 s、
  ...
  的余量 6.1% → 48%，三次运行值一致），完整验收 PASS（V/P 切换
  0.8944 / 1.300 MPa 由压力判据触发，闸口封冻与顶出判据不变）；
