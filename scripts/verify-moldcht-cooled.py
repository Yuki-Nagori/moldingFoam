...
HTC = 2000.0         # coolant HTC [W/m^2/K]
TA = 300.0           # coolant temperature [K]
ATOP = 0.06*0.002    # mould top area [m^2]
...
def main():
    ...
    for t in times:
        ...
        ttop = patch_mean(
            os.path.join(case_dir, time_dir(t), "mold", "T"), "moldTop")
        samples.append((t, ec + em, tvent, avent, alpha, ttop))
    ...
    for i in range(1, len(samples)):
        t0, e_prev, tv0, av0, a0, tt0 = samples[i - 1]
        t1, e_now, tv1, av1, a1, tt1 = samples[i]
        dt = t1 - t0
        ...
        # Convective cooling through the mould top
        qcool += HTC*ATOP*0.5*((tt0 - TA) + (tt1 - TA))*dt

        qin = MDOT*CVM*(TIN - TREF)*t1
        expected = qin - qout - qcool
        de = e_now - e0
    ...
    print("  integrated cooling = {:.3f} J".format(qcool))
