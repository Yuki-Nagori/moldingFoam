...
| `moldCHT-cycle` | 多周期多区域 CHT：4 周期模温 354→419 K，每周期增量 22.7→11.3 K 递减（`verify-moldcht-cycle.py`，`xmake run moldCHT`） |
...
- `validation/moldCHT-cycle`：多周期集成（cavity 周期重置、模具温度
  保留）：4 个周期完成，模具平均温度 354.1 → 419.2 K，每周期增量
  22.7 → 12.3 → 11.3 K 递减（周期稳态趋势），界面连续；
