...
| [070](tasks/070-structural-convergence-repair.md) | 结构基准迭代收敛与细网格精度修复 | P1 | in-progress（S0/S2 已完成单变量取证；细网格静态收敛与根因修复未完成） |
| [071](tasks/071-solid-displacement-corrector-convergence.md) | solidDisplacement 校正循环的真实收敛控制 | P1 | planned（已确认上游首轮残差复用；待评估模块覆写或上游补丁入口） |
...
#### 补充执行顺序（2026-09-15，069/070）

上表为历史专项顺序；本轮测量异常按以下流程跟进：

| 顺序 | 任务/阶段 | 交付与退出条件 |
|---|---|---|
| 1 | 070 S0–S1 + 069 H1–H2 | 复读现场、核实 steadyState/迭代语义；采样自证、阶段超时留证与分层判定 |
| 2 | 070 S2–S3 | 单变量比较迭代预算、relTol、加速；根因证据及静态收敛硬判据 |
| 3 | 070 S4 + 069 H3 | 结构三网格满足精度；补压力/热通量/取向独立瞬态研究，逐量提交 |
| 4 | 069 H4–H5 | 可适用的收敛分析、完整 nightly 实测与 artifact；附提交和精度/性能对照后收口 |

070 复用 069 已交付的工具，无需等待 069 done；069 的结构验收消费 070
结果，非结构部分可独立推进。两个结构算例使用 steadyState，现有 deltaT
组合不能视为物理时间离散研究。流程设计已完成，修复实施与新验收尚未完成。
